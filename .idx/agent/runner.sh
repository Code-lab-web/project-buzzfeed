...
python3 "$SCRIPT_DIR/repo-scanner/scanner.py" "$CLONE_DIR" > "$REPO_SUMMARY"
...
python3 "$SCRIPT_DIR/llm-client/generate_full_docs.py" "$REPO_SUMMARY" "$CLONE_DIR"
echo "Full documentation generated."
# Commit and open PR
bash "$SCRIPT_DIR/commit_and_pr.sh" "$CLONE_DIR" "$REPO_NAME"
echo "PR creation completed!"
echo "Please manually register the catalog-info.yaml in Developer Hub after PR is merged."