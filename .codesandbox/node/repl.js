...
const toDynamicImport = (codeLine) => {
  ...
  acornWalk.ancestor(ast, {
    ImportDeclaration(node) {
      const awaitDynamicImport = `await import(${JSONStringify(
        node.source.value
      )});`;
        ...
        const importNames = ArrayPrototypeJoin(
          ArrayPrototypeMap(node.specifiers, ({ local, imported }) =>
            local.name === imported?.name
              ? local.name
              : `${imported?.name ?? "default"}: ${local.name}`
          ),
          ", "
        );
...
class REPLServer extends Interface {
  constructor(prompt, stream, eval_, useGlobal, ignoreUndefined, replMode) {
      ...
      if (experimentalREPLAwait && StringPrototypeIncludes(code, "await")) {
        ...
        try {
          ...
          if (e.name === "SyntaxError") {
            ...
            const fallbackCode = SideEffectFreeRegExpPrototypeSymbolReplace(
              /\bawait\b/g,
              code,
              ""
            );
            ...
            if (e.name === "SyntaxError") {
              ...
              if (StringPrototypeIncludes(e.message, importErrorStr)) {
                e.message =
                  ...
                  toDynamicImport(ArrayPrototypeAt(self.lines, -1));
                e.stack = SideEffectFreeRegExpPrototypeSymbolReplace(
                  /SyntaxError:.*\n/,
                  e.stack,
    ...
    self.on("line", function onLine(cmd) {
      ...
      if (self.editorMode) {
        ...
        // code alignment
        const matches =
          self._sawKeyPress && !self[kLoadingSymbol]
            ? RegExpPrototypeExec(/^\s+/, cmd)
            : null;
        if (matches) {
      ...
      if (trimmedCmd) {
        if (
          ...
          const matches = RegExpPrototypeExec(
            /^\.([^\s]+)\s*(.*)$/,
            trimmedCmd
          );
          ...
          if (
            FunctionPrototypeCall(_parseREPLKeyword, self, keyword, rest) ===
            true
          ) {
            return;
          }
  ...
  close() {
    if (
      this.terminal &&
      this.historyManager.isFlushing &&
      !this._closingOnFlush
    ) {
